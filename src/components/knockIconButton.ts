...
const addIconStyles: svgStylesType[] = [{elementType: 'path', 'd': 'M10 4.16675V15.8334', 'stroke': '#94A3B8', 'stroke-width': '2', 'stroke-linecap': 'round', 'stroke-linejoin': 'round'}, {elementType: 'path', 'd': 'M4.16699 10H15.8337', 'stroke': '#94A3B8', 'stroke-width': '2', 'stroke-linecap': 'round', 'stroke-linejoin': 'round'}];
...
const getIconClassList = function(iconName: string): null|string[]{
    if(iconName === 'spinIcon'){
        return ['km-flex-none', 'km-animate-spin'];
    } else{
        return ['km-flex-none'];
    };
};
...
const generateCircleIconButton = function(iconName: string, iconClickEvent: any): HTMLElement{
    const svgStlyes = getIconStyles(iconName);
    const svgClassList = getIconClassList(iconName);

    const iconButtonElement: HTMLElement = generateNodeElement('button', ['km-w-9', 'km-h-9', 'km-bg-slate-50', 'km-rounded-full', 'km-flex', 'km-justify-center', 'km-items-center', 'km-flex-none', 'knockCursorPointer', 'km-select-none', '[@media(pointer:fine){&:hover}]:km-bg-slate-100', 'active:km-bg-slate-100', 'km-transition-[background-color]']);
...
const generateSquareIconButton = function(iconName: string, iconClickEvent: any): HTMLElement{
    const svgStlyes = getIconStyles(iconName);
    const svgClassList = getIconClassList(iconName);

    const iconButtonElement: HTMLElement = generateNodeElement('button', ['km-w-14', 'km-h-14', 'km-bg-white', 'km-border', 'km-border-solid', 'km-border-slate-200', 'km-rounded-2xl', 'km-flex', 'km-justify-center', 'km-items-center', 'km-flex-none', 'knockCursorPointer', 'km-select-none', 'km-transition-colors', '[@media(pointer:fine){&:hover}]:km-border-slate-400', 'active:km-border-slate-400']);
    const svgElement: SVGElement = generateSvgElement(svgClassList, svgStlyes);
    appendElements(iconButtonElement, [svgElement]);
    iconButtonElement.addEventListener('click', () => {iconClickEvent()});
    return iconButtonElement;
...
export { generateCircleIconButton, generateSquareIconButton }